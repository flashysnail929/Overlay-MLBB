...
export default function PostDraftCard({ side }: PostDraftCardProps) {
  ...
  return (
    <div className="w-full flex-5/12 h-full flex flex-col gap-6">
      {/* Team Name */}
      <div className="h-2/12 w-full flex flex-row items-center justify-between">
        {/* maybe team logo */}

        {/* Team Name */}
        <div className="w-full text-6xl font-semibold uppercase">
          <h2 className={`${side === "left" ? "text-left" : "text-right"}`}>
            {side === "left" ? leftTeam.name : rightTeam.name}
          </h2>
        </div>
      </div>
      ...
      {/* Team Pick */}
      <div
        className={`h-10/12 flex items-center justify-between bg-slate-950/90 border
        ...
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          return (
            ...
            >
                  ...
                  src={
                    side === "left"
                      ...
                      : rightHeroSlug === "idle"
                        ? rightPickIdlePic
                        : heroImages[
                            `/src/assets/HeroPick/${rightHeroSlug}.png`
                          ]
                  }
                  alt={`${side}-heroImg-${i}`}
              ...
              <div
                className={`h-[clamp(34px,3.2vw,44px)] text-center flex flex-col justify-center px-1 border-t backdrop-blur-md relative ${
                  side === "left"
